...
$$ 
y_i^{(1)} \sim \text{Poisson}(\lambda\exp(\rho))\\
$$
...
$$
where we summarize all hyperparameters as $\theta = [\alpha_\lambda, \beta_\lambda, \alpha_\rho, \beta_\rho]$ to avoid clutter. As we see in the the next section, we would actually like to reason about the posterior $p(\lambda, |\mathbb{D}, \theta)$. 
