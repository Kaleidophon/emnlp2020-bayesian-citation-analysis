...
get_weakly_informative_normal = function(samples_short, samples_long) {
  lower = min(min(samples_short + 1) / min(samples_long + 1), min(samples_long + 1) / min(samples_short + 1)) - 1
  upper = max(max(samples_short) / max(samples_long), max(samples_long) / max(samples_short))
  mu = (lower + upper) / 2
