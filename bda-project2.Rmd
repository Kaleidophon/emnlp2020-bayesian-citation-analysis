...
simple_non <- stan(file = "model_simple_non.stan", 
...
```{r}
simple_weak <- stan(file = "model_simple_weak.stan", 
            data = simple_model_data,
            chains = 4,
            iter = 3000,
            warmup = 2000)
...
seperate <- stan(file = "stan_models/citation-length-seperate.stan", 
            data = seperate_data,
            chains = 4,
            iter = 4000,
            warmup = 2000)
...
```{r}
draws <- as.data.frame(seperate)

# group together diff. indicies e.g. lambda[1], lambda[2] -> lambda[1,1]
params <- extract(seperate, permuted=TRUE)
